import { useCreateServerMutation } from "../../../../../features/server/slice/serversApiSlice";
import "./embedsInput.scss";
import { Close } from "@mui/icons-material";
import { getDownloadURL, ref, uploadBytes } from "firebase/storage";
import { storage } from "../../../../../lib/firebase/config/firebase";
import { useState } from "react";
import { serverThumbnailDelete } from "../../../main/components/forum/components/singlePost/components/hooks/deleteImage";
...
const EmbedsInput = ({
  ...
  const [clientSideImgSrc, setClientSideImgSrc] = useState("");
  ...
  const encodeFileToBase64 = (fileBlob) => {
    ...
    return new Promise((resolve) => {
      reader.onload = () => {
        setClientSideImgSrc(reader.result);
        resolve();
      };
    });
  };
      ...
      .then((snapshot) => {
        getDownloadURL(snapshot.ref)
          .then((url) => {
            thumbnail = url;
            if (thumbnail) {
              createServer({ initialServer, thumbnail })
              .unwrap()
              .catch((err) => {
                console.error(err);
                setInitialServer({
  ...
  return (
    <div className="server-embedInput">
      <div className="wrapper">
        <button className="topRightBtn">
        ...
        <div className="iconContainer">
          {file && clientSideImgSrc ? (
            <>
              <label htmlFor="server_changed_thumbnail">
                <img src={clientSideImgSrc} alt="" />
              </label>
              <input
                id="server_changed_thumbnail"
                type="file"
                onChange={(e) => {
            ...
            <>
              <label htmlFor="server_thumbnail">
        ...
        <div className="bottom">
          ...
          <p>
            서버를 만들면 DevBoard의 <span>커뮤니티 지침</span>에 동의하게
            됩니다.
          </p>
          <h2>서버 소개</h2>
          ...
          <section>
            <div className="sectionWrap">
              <span onClick={handlePrevPage}>뒤로 가기</span>
